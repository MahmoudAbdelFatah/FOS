...
//2017
//Reset the CNT0 to its initial value without affecting the interrupt status
void kclock_reset(void)
{
	kclock_write_cnt0_LSB_first(CLOCK_INTERVAL_IN_CNTS) ;
}
//==============
void
kclock_resume(void)
{
	...
	outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
	//2017: if the remaining time =1, then increase it a bit to avoid invoking the CLOCK INT
