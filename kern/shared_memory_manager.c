...
int createSharedObject(int32 ownerID, char* shareName, uint32 size,uint8 isWritable, void* virtual_address) {
	...
	uint32 *env_ptr_page_dir = myenv->env_page_directory;
	...
	int ShareObjID = allocate_share_object(&shareObj);
	//	2) Allocate the required space in the physical memory on a PAGE boundary @allocate_frame
	if (ShareObjID != E_NO_SHARE) {
		if (virtual_address != NULL) {
			...
			for (int i = 0; i < pageCnt; i++) {
				...
				if (ret != E_NO_MEM) {
					...
					map_frame(env_ptr_page_dir, frameInfo,
							virtual_address + (i * PAGE_SIZE), perm);
...
int getSizeOfSharedObject(int32 ownerID, char* shareName) {
	...
	//
	int ShareObjIndex = get_share_object_ID(ownerID, shareName);
	if (ShareObjIndex != E_SHARED_MEM_NOT_EXISTS) {
		struct Share *shareObj = &shares[ShareObjIndex];
		return shareObj->size;
	} else {
		return E_SHARED_MEM_NOT_EXISTS;
	}
	//change this "return" according to your answer
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address) {
	...
	if (ShareObjIndex != E_SHARED_MEM_NOT_EXISTS) {
		...
		int framesCnt = ROUNDUP(shareObj->size,PAGE_SIZE) / PAGE_SIZE;
		for (int i = 0; i < framesCnt; i++) {
			struct Frame_Info* frame = get_frame_from_storage(
					shareObj->framesStorage, i);
			int perm = PERM_PRESENT | PERM_USER;
			if (shareObj->isWritable == 1) {
		...
		//  4) make sure that read-only object must be shared "read only",
		//      use the flag isWritable to make it either read-only or writable
		//	5) Update references
		shareObj->references++;
		// 	6) 	If succeed: return the ID of the shared object (i.e. its index in the "shares" array)
		//		Else, return suitable error
		return ShareObjIndex;
	} else {
		return E_SHARED_MEM_NOT_EXISTS;
	}
	//change this "return" according to your answer
